...
namespace WindowsSDK
{
    ...
    {
        public List<payment> sp_get_all_authorizations()
        {
            #region Check-for-Null-Values

            if (string_null_or_empty(_token_string))
            {
                log("sp_get_all_authorizations null value detected for token, please authenticate", true);
                return null;
            }
            ...
            get_authorization_rest_resp = rest_client<payment>(
                _endpoint_url + "payment?auto_capture=0",
                "GET",
                null,
                null);
            ...
            {
                log("sp_get_all_authorizations null response from rest_client for authorization retrieval call", true);
                return null;
            }

            if (get_authorization_rest_resp.status_code != 200)
            {
                log("sp_get_all_authorizations rest_client returned status other than 200 for authorization retrieval call", true);
                return null;
            }
            ...
            {
                log("sp_get_all_authorizations unable to deserialize response from server for authorization retrieval call", true);
                return null;
            }

            if (!get_authorization_resp.success)
            {
                log("sp_get_all_authorizations success false returned from server for authorization retrieval call", true);
                return null;
            }
            ...
            try
            {
                ...
                log("sp_get_all_authorizations response retrieved");
            }
            catch (Exception)
            {
                log("sp_get_all_authorizations unable to deserialize authorization list object", true);
                return null;
            }

            if (curr_authorization_list == null)
            {
                log("sp_get_all_authorizations null authorization list retrieved", true);
                return null;
            }

            if (curr_authorization_list.Count < 1)
            {
                log("sp_get_all_authorizations no authorizations retrieved", true);
                return null;
            }

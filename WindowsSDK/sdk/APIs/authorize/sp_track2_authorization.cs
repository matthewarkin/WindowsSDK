...
namespace WindowsSDK
{
    ...
    {
        public object sp_track2_authorization(
            string track2,
            string notes,
            string latitude,
            string longitude,
            decimal amount)
        {
            ...
            {
                log("sp_track2_authorization null value detected for token, please authenticate", true);
                return null;
            }
            ...
            {
                log("sp_track2_authorization null value detected in one of the input values", true);
                return null;
            }

            if (amount <= 0)
            {
                log("sp_track2_authorization amount must be greater than zero", true);
                return null;
            }
            ...
            {
                log("sp_track2_authorization null response from rest_client for simple authorization call", true);
                return null;
            }
            ...
            {
                log("sp_track2_authorization unable to deserialize response from server for simple authorization call", true);
                return null;
            }
            ...
            try
            {
                ...
                log("sp_track2_authorization response retrieved");
            }
            catch (Exception)
            {
                log("sp_track2_authorization unable to deserialize processor response", true);
                curr_resp = null;
            }
            ...
            if (curr_resp == null)
            {
                try
                {
                    ...
                    log("sp_track2_authorization error_message detected: " + error.error_code + " " + error.error_file + " " + error.error_text);
                    return error;
                }
                catch (Exception)
                {
                    log("sp_track2_authorization could not deserialize as processor_cc_txn_response or error_message, returning null");
                    return null;
                }
            }
